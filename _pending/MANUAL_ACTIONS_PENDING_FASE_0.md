...
0. **Aplicar 5 migs de Fase 2 scaffolding (2026-04-27)** — ver Supabase Dashboard → SQL Editor:
   ...
   - [supabase/migrations/20260903800000_timeline_auto_sync_triggers.sql](../supabase/migrations/20260903800000_timeline_auto_sync_triggers.sql) — §2.4.3 CRÍTICO: 4 triggers que sincronizan medical_records / pet_reminders (completed) / vet_bookings (completed) / routine_completions → pet_timeline_events. Sin esto el timeline solo recibe inserts manuales. Cada trigger SECURITY DEFINER + EXCEPTION WHEN OTHERS + idempotency.
   - [supabase/migrations/20260903900000_rename_orphan_tables.sql](../supabase/migrations/20260903900000_rename_orphan_tables.sql) — §9.0.bis.5 renames a `_deprecated_20260427`: `comprehensive_medical_records`, `vet_pet_relationships`, `points_history`, `virtual_routes`. Skip si no existen. NO drop — solo rename para detectar usos en runtime.
   - [supabase/migrations/20260904000000_table_audit_rpc.sql](../supabase/migrations/20260904000000_table_audit_rpc.sql) — §9.0.bis.4 RPC `audit_table_health()` para ritual mensual de limpieza (§2.10.3). Clasifica tablas: zombie (0 filas) / tiny / dormant / stale / deprecated / active.
