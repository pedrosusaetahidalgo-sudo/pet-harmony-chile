...
async function handle(req: Request): Promise<Response> {
  ...
  if (userIds.length === 0) return errorResponse('user_ids required', 400);
  ...
  // Cargar device_tokens habilitados de los users requested
  ...
  if (tokens.length === 0) {
    return jsonResponse({ ok: 0, skipped: userIds.length, reason: 'no_device_tokens' });
  }
  ...
  return jsonResponse({
    ok: androidResult.ok + iosResult.ok,
