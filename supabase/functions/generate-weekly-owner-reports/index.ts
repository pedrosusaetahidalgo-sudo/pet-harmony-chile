...
serve(
  withTelemetry('generate-weekly-owner-reports', async (req) => {
    ...
    try {
      ...
      for (const owner of validOwners) {
        try {
          ...
          const completedReminders = remindersLastWeek?.filter((r) => r.is_completed).length ?? 0;
          ...
          const reportContent = {
            ...
            reminders: {
              ...
              completion_rate:
                totalReminders > 0 ? Math.round((completedReminders / totalReminders) * 100) : null,
            },
          ...
          if (apiKey) {
            try {
              ...
              console.warn(`[weekly-owner-reports] AI insight failed for user ${userId}:`, aiErr);
            }
          }
          ...
          if (insertErr) {
            console.error(`[weekly-owner-reports] insert failed for user ${userId}:`, insertErr);
            errors++;
          } else {
            generated++;
          }
        } catch (userErr) {
          console.error(`[weekly-owner-reports] error for user ${owner.id}:`, userErr);
          errors++;
        }
      }

      return new Response(JSON.stringify({ generated, errors, total_users: validOwners.length }), {
        status: 200,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    } catch (error) {
