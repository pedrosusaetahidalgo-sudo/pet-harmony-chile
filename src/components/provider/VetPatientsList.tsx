    ...
    queryFn: async () => {
      ...
      if (notes && Array.isArray(notes)) {
        for (const note of notes) {
          ...
          const pet = note.pets as { name: string | null; species: string | null; photo_url: string | null; owner_id: string | null } | null;
          patientsMap.set(pid, {
            pet_id: pid,
      ...
      if (providerRow?.id) {
        ...
        if (shared && Array.isArray(shared)) {
          for (const row of shared as any[]) {
            ...
            if (patientsMap.has(pid)) continue;
            const pet = row.pets as { name: string | null; species: string | null; photo_url: string | null; owner_id: string | null } | null;
            patientsMap.set(pid, {
              pet_id: pid,
              pet_name: pet?.name || "Mascota",
              species: pet?.species || null,
      ...
      const allPets = [...(notesList || []).map((n: any) => n.pets), ...([] as any[])];
              ...
              return (
                ...
                >
                  ...
                  <div className="flex gap-1.5 flex-shrink-0">
                    <Link to={`/pet/${patient.pet_id}/clinical`}>
