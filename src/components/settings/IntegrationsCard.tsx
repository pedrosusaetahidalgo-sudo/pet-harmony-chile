...
export function IntegrationsCard() {
  ...
  const handleSaveWhatsApp = async () => {
    ...
    if (whatsappOptedIn && !whatsappNumber.match(/^\+?56\s?9\s?\d{4}\s?\d{4}$/)) {
      setWhatsappError('Número inválido. Usa formato +569 1234 5678');
      return;
    }
    setWhatsappError('');
  ...
  const handleDisconnectCalendar = async () => {
    if (
      !confirm(
        '¿Seguro que quieres desconectar Google Calendar? Tu calendario "Paw Friend" quedará en tu Google, puedes borrarlo manualmente si quieres.'
      )
    )
      return;
        ...
        <div className="space-y-3">
          ...
          <div className="pl-12 space-y-2">
            {!calendarConnected && (
              <div className="rounded-md bg-blue-50 border border-blue-200 px-3 py-2 text-xs text-blue-800">
                <strong>Nota:</strong> Google mostrará una pantalla de permisos de Paw Friend. Solo
                accederemos a tus eventos de calendario para sincronizar citas y recordatorios. No
                leemos ni modificamos ningún otro dato de tu cuenta.
              </div>
            )}
          ...
          {!calendarConnected && (
            <div className="pl-12 flex items-start gap-2 text-xs text-muted-foreground">
              ...
              <span>
                Al conectar, creamos un calendario llamado &ldquo;Paw Friend&rdquo; en tu Google con
                todos tus eventos. Puedes desconectar cuando quieras.
              </span>
            </div>
          )}
        </div>
