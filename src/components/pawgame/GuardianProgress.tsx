      ...
      <CardContent className="p-4 md:p-6">
        <div className="flex flex-col md:flex-row items-start md:items-center gap-4">
          ...
          <div className="flex items-center gap-4">
            <div
              className={`relative p-4 rounded-2xl bg-gradient-to-br ${getLevelGradient(level)} shadow-lg`}
            >
              {getLevelIcon(level)}
            ...
            <div>
              <h3 className="font-bold text-lg">
                {currentLevel?.level_name || getTier(level).name}
              </h3>
              <p className="text-sm text-muted-foreground">
                {currentLevel?.description || 'Comenzando tu aventura'}
              </p>
              {multiplier > 1 && (
                <Badge variant="secondary" className="mt-1 bg-yellow-500/10 text-yellow-600">
                  <Zap className="h-3 w-3 mr-1" />x{multiplier} bonus
                </Badge>
              )}
            </div>
          </div>
          ...
          <div className="flex-1 w-full md:w-auto">
            ...
            <div className="mb-3">
              ...
              <Progress value={Math.min(progressPercent, 100)} className="h-3 bg-muted" />
            </div>
