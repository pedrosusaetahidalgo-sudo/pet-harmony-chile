...
export const PawShopRewards = ({ userPoints, userId, onPurchase }: PawShopRewardsProps) => {
  ...
  const handlePurchase = async (reward: ShopReward) => {
    ...
    try {
      ...
      const { error: pointsError } = await supabase.from('paw_point_transactions').insert({
        user_id: userId,
  ...
  const renderRewardCard = (reward: ShopReward) => {
    const Icon = reward.service_type
      ? getServiceIcon(reward.service_type)
      ...
      >
        {reward.discount_percentage && (
          <div className="absolute top-2 right-2">
            <Badge className="bg-red-500 text-white">-{reward.discount_percentage}%</Badge>
          </div>
        )}
        ...
        <CardContent className="p-4">
          <div className="flex flex-col items-center text-center">
            ...
            {reward.partner_name && (
              <p className="text-[10px] text-muted-foreground mb-1">por {reward.partner_name}</p>
            )}
