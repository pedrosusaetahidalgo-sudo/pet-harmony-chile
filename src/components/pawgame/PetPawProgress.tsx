  ...
  const calculateOverallScore = (progress: PetProgress | undefined) => {
    ...
    return Math.round(
      (progress.health_score +
        progress.activity_score +
        progress.happiness_score +
          ...
          return (
            <Card key={pet.id} className="overflow-hidden hover:shadow-lg transition-shadow">
              ...
              <CardContent className="space-y-4">
                ...
                {(() => {
                  const isUpToDate =
                    pet.vaccination_status === 'up_to_date' || progress?.vaccines_up_to_date;
                  return (
                    <div className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                      <div className="flex items-center gap-2">
                        <Syringe
                          className={`h-4 w-4 ${isUpToDate ? 'text-green-500' : 'text-orange-500'}`}
                        />
