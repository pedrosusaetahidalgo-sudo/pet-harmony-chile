  ...
  const { data: report } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await (supabase as any)
        ...
        .limit(1)
        .maybeSingle();
      return data as { id: string; report_type: string; period_start: string; period_end: string; content_jsonb: Record<string, unknown> } | null;
    },
    enabled: !!user?.id,
    staleTime: 5 * 60 * 1000,
  });
    ...
    <Card className="border-l-4 border-l-purple-600 bg-purple-50/50">
      ...
      <CardContent className="space-y-2">
        {highlights.slice(0, 3).map((h, i) => (
          <p key={i} className="text-sm text-purple-800">{h}</p>
        ))}
        {insight && (
