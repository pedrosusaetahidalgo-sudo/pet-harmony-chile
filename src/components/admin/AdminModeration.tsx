...
export default function AdminModeration() {
  ...
  const resolveReport = async (id: string, action: "dismissed" | "removed") => {
    ...
    if (action === "removed") {
      ...
      if (report && report.report_type === "post") {
        await supabase.from("posts").delete().eq("id", report.reported_id);
      }
    }

    if (!error) {
      toast({ title: action === "removed" ? "Contenido eliminado" : "Reporte descartado" });
      ...
      {isLoading ? (
        ...
        <Card className="border-dashed">
          <CardContent className="py-8 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">Todos los reportes han sido atendidos.</p>
          </CardContent>
        </Card>
      ) : (
              ...
              <CardContent className="p-4 flex items-center justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <Badge variant="outline" className="text-xs capitalize">{r.report_type}</Badge>
                  ...
                  <p className="text-xs text-muted-foreground mt-1 font-mono">ID: {r.reported_id.slice(0, 8)}...</p>
                </div>
                {r.status === "pending" && (
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <Button variant="destructive" size="sm" onClick={() => resolveReport(r.id, "removed")}>
