...
export default function ProDashboard() {
  ...
  const comparisonData = useMemo(() => {
    ...
    return [
      { metric: "Recordatorios", current: current.remindersCompleted, previous: previous.remindersCompleted },
    ...
    <div className="container max-w-4xl mx-auto p-4 md:p-6 space-y-4 md:space-y-6 animate-fade-in">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Analytics detallados de tus mascotas
        </p>
      </div>
          ...
          <CardContent>
            {analytics && analytics.activityTimeline.length > 1 ? (
              <ChartContainer config={activityChartConfig} className="h-[200px] w-full aspect-auto">
                <AreaChart data={analytics.activityTimeline} margin={{ top: 5, right: 5, bottom: 0, left: -20 }}>
        ...
        <Card>
          ...
          <CardContent>
            {comparisonData.length > 0 ? (
              <ChartContainer config={comparisonChartConfig} className="h-[200px] w-full aspect-auto">
