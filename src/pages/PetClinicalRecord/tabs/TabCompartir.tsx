...
export function TabCompartir({ petId, petName }: { petId: string; petName: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const map = new Map((profiles || []).map((p) => [p.id, p.display_name || 'Veterinario/a']));
      const merged: DirectoryVet[] = providers.map(
        (p: { id: string; slug: string | null; user_id: string | null }) => ({
          id: p.id,
          slug: p.slug,
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Genera un enlace seguro para compartir la ficha clínica de tu mascota con un
            veterinario. Los enlaces expiran automáticamente después de 30 días.
          </p>
