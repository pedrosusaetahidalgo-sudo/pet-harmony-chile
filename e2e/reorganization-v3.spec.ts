 ...
 *
 * Protege los cambios hechos en la v3:
 *  - /calendario acepta ?tab=hoy|recordatorios|rutinas|reservas (Paso 9)
 *  - /services/vets redirige a /veterinarios (Paso 3)
 *  - BottomTabBar owner tiene Inicio · My Paws · Servicios · Agenda · Perfil (Paso 14)
...
test.describe('Reordenamiento v3 — /calendario tabs', () => {
  ...
  test('4 tabs presentes: Hoy · Recordatorios · Rutinas · Reservas', async ({ page }) => {
    await injectFakeAuth(page);
    await page.goto('/calendario', { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(500);
...
test.describe('Reordenamiento v3 — BottomTabBar owner (mobile)', () => {
  ...
  test('tabs nuevos: Inicio · My Paws · Servicios · Agenda · Perfil', async ({ page }) => {
    await injectFakeAuth(page);
    ...
    await expect(nav.getByText('My Paws')).toBeVisible();
